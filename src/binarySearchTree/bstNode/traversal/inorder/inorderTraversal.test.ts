...
describe(`Inorder traversal of BST`, () => {
  ...
  describe(`To array..`, () => {
    ...
    it('Should return empty array, if BST is empty', () => {
      const result: number[] = inorderTraversal.apply(null, [toArray, []]);
      expectChai(result)
    ...
    it('Should return exact array (sorted array), if BST is not empty', () => {
      const result: number[] = inorderTraversal.apply(mockBstNode, [toArray, []]);
      const expected: number[] = [8, 10, 12, 15, 17, 20, 25];
      expectChai(result)
        .to.be.an([].constructor.name);
      expectChai(result)
  ...
  describe(`Sum..`, () => {
    ...
    it('Should return 0, if BST is empty', () => {
      const result: number = inorderTraversal.apply(null, [sum, 0]);
      expectChai(result)
        .to.be.equal(0);
    });

    it('Should return bst node sum', () => {
      const result: number = inorderTraversal.apply(mockBstNode, [sum, 0]);
      expectChai(result)
        .to.be.equal(107);
    });
  });

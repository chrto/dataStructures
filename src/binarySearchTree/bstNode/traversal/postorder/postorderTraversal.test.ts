...
/*
             ...
             8    12   17   25
*/
describe(`Postorder traversal of BST`, () => {
  ...
  describe(`To array..`, () => {
    ...
    it('Should return empty array, if BST is empty', () => {
      const result: number[] = postorderTraversal.apply(null, [toArray, []]);
      expectChai(result)
    ...
    it('Should return exact array, if BST is not empty', () => {
      const result: number[] = postorderTraversal.apply(mockBstNode, [toArray, []]);
      const expected: number[] = [8, 12, 10, 17, 25, 20, 15];
  ...
  describe(`Sum..`, () => {
    ...
    it('Should return 0, if BST is empty', () => {
      const result: number = postorderTraversal.apply(null, [sum, 0]);
      expectChai(result)
        .to.be.equal(0);
    });

    it('Should return bst node sum', () => {
      const result: number = postorderTraversal.apply(mockBstNode, [sum, 0]);
      expectChai(result)
        .to.be.equal(107);
    });
  });
